...
for cache_ty in cache_types:
	if cache_ty == "unified":
		for block_size in cache_block_size:
				...
				for assoc1 in associativity:# Associativity for L1
					...
					for rep_p2 in replace_polys:# replacement policy for L2
						...
						for assoc2 in associativity:# Associativity for L2
							...
							num = num +1
							print num
							os.system("echo " +str(num)+'  l1 :'+str(L1u_sets)+':'+block_size+':'+assoc1+':'+rep_p1+' l2:'+str(L2u_sets)+':'+block_size+':'+assoc2+':'+rep_p2+'>> ./output/ANAGRAM_L1L2uni.txt')
							...
							os.system('cat output_file.txt | grep -a "sim_num_insn" >> ./output/ANAGRAM_L1L2uni.txt')
							os.system('cat output_file.txt | grep -a "ul1.accesses" >> ./output/ANAGRAM_L1L2uni.txt')
							os.system('cat output_file.txt | grep -a "ul1.miss_rate" >> ./output/ANAGRAM_L1L2uni.txt')
							os.system('cat output_file.txt | grep -a "ul2.accesses" >> ./output/ANAGRAM_L1L2uni.txt')
							os.system('cat output_file.txt | grep -a "ul2.miss_rate" >> ./output/ANAGRAM_L1L2uni.txt')
